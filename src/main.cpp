...
int main() {
    ...
    Coinbase::CoinbaseClient cclient = Coinbase::CoinbaseClient(env_reader, RealClock());
    auto items = kclient.getTradeInfo({"TG3PXU-WIYH3-ZJY44V", "TUBBK6-6CPHX-K4S3UO"});
    Kraken::KrakenLoader loader(kclient, "exchange_files");
    const std::chrono::hours currentHour = std::chrono::floor<std::chrono::hours>(std::chrono::system_clock::now().time_since_epoch());
